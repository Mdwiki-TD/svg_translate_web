...
CREATE TABLE `owid_charts_templates` (`chart_id` int(11), `template_id` int(11), `template_title` varchar(255));


DROP TABLE IF EXISTS `owid_charts_templates`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `owid_charts_templates` AS select `c`.`chart_id` AS `chart_id`,`t`.`id` AS `template_id`,`t`.`title` AS `template_title` from (`owid_charts` `c` left join `templates` `t` on(`t`.`slug` = `c`.`slug`));

CREATE TABLE `templates_need_update` (`template_id` int(11), `template_title` varchar(255), `slug` varchar(255), `max_time` int(11), `last_world_year` int(11));


DROP TABLE IF EXISTS `templates_need_update`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `templates_need_update` AS select `t`.`id` AS `template_id`,`t`.`title` AS `template_title`,`t`.`slug` AS `slug`,`c`.`max_time` AS `max_time`,`t`.`last_world_year` AS `last_world_year` from (`owid_charts` `c` join `templates` `t` on(`t`.`slug` = `c`.`slug`)) where `t`.`last_world_year` <> `c`.`max_time` and `t`.`last_world_year` is not null;
