...
class TestFixNestedJobsProcessorSteps(TestSetup):
    def test_verify_step_success(self, mock_fix_nested_services, tmp_path) -> None:
        ...
        result = self.processor._verify_step()

        assert result is True
        assert self.processor.result.stages.verify.status == "success"

    def test_verify_step_failure_no_tags_fixed(self, mock_fix_nested_services, tmp_path) -> None:
        self.processor.result.stages.fix.status = "success"
        self.processor.result.file_result = FileResult(path=str(tmp_path / "test.svg"), nested_tags_before=2)
    ...
    def test_upload_step_success(self, mock_fix_nested_services, tmp_path) -> None:
    ...
    def test_upload_step_failure(self, mock_fix_nested_services, tmp_path) -> None:
...
class TestFixNestedJobsProcessor(TestSetup):
    ...
    def test_is_cancelled_no_event(self, mock_fix_nested_services) -> None:
...
class TestIsCancelled(TestSetup):
    def test_returns_false_when_not_cancelled(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = False
        proc = self.processor
        assert proc.is_cancelled() is False

    def test_cancel_event_set_returns_true(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = False
    ...
    def test_jobs_service_cancelled_returns_true(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = True
        proc = self.processor
        assert proc.is_cancelled(check_db=True) is True

    def test_sets_result_status_to_cancelled(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = True
        proc = self.processor
        proc.is_cancelled(check_db=True)
        assert proc.result.status == "cancelled"

    def test_sets_cancelled_at_timestamp(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = True
        proc = self.processor
        proc.is_cancelled(check_db=True)
        assert proc.result.cancelled_at is not None

    def test_does_not_overwrite_existing_cancelled_at(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = True
        proc = self.processor
    ...
    def test_updates_stage_status_when_stage_name_given(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = True
        proc = self.processor
    ...
    def test_ignores_unknown_stage_name(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = True
        proc = self.processor
        # should not raise
...
class TestAnalyzeStep(TestSetup):
    ...
    def test_skips_when_download_not_success(self, mock_fix_nested_services):
        proc = self.processor
    ...
    def test_returns_false_when_file_missing(self, mock_fix_nested_services, tmp_path):
        proc = self._proc_with_download_success(tmp_path / "missing.svg")
        result = proc._analyze_step()
        assert result is False
        mock_fix_nested_services["detect_nested_tags"].assert_not_called()

    def test_returns_none_when_no_nested_tags(self, mock_fix_nested_services, tmp_path):
        svg = tmp_path / "a.svg"
        svg.touch()
    ...
    def test_returns_true_when_nested_tags_found(self, mock_fix_nested_services, tmp_path):
        svg = tmp_path / "b.svg"
        svg.touch()
...
class TestFixStep(TestSetup):
    ...
    def test_skips_when_analyze_not_success(self, mock_fix_nested_services):
        proc = self.processor
...
class TestVerifyStep(TestSetup):
    ...
    def test_skips_when_fix_not_success(self, mock_fix_nested_services):
        proc = self.processor
    ...
    def test_returns_true_when_tags_fixed(self, mock_fix_nested_services, tmp_path):
    ...
    def test_returns_false_when_no_tags_fixed(self, mock_fix_nested_services, tmp_path):
...
class TestUploadStep(TestSetup):
    ...
    def test_skips_when_upload_disabled(self, mock_fix_nested_services, tmp_path):
...
class TestRunStage(TestSetup):
    def test_returns_true_when_step_returns_true(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = False
        proc = self.processor
        assert proc._run_step(proc.result.stages.download, lambda: True) is True

    def test_returns_false_and_sets_failed_when_step_returns_false(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = False
        proc = self.processor
        assert proc._run_step(proc.result.stages.download, lambda: False) is False
        assert proc.result.status == "failed"

    def test_returns_false_and_sets_skipped_when_step_returns_none(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = False
        proc = self.processor
        assert proc._run_step(proc.result.stages.download, lambda: None) is False
        assert proc.result.status == "skipped"

    def test_handles_exception_and_sets_failed(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = False
        proc = self.processor
    ...
    def test_returns_false_immediately_when_cancelled(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = True
    ...
    def test_sets_stage_status_to_running_before_calling_step(self, mock_fix_nested_services):
        mock_fix_nested_services["is_job_cancelled"].return_value = False
        statuses: list = []
...
class TestRun(TestSetup):
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, mock_before_run, tmp_path):
