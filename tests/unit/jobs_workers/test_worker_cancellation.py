...
def test_collect_templates_data_worker_cancellation(mock_common_services, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "src.main_app.jobs_workers.collect_templates_data_worker.get_category_members",
    ...
    monkeypatch.setattr(
        "src.main_app.jobs_workers.collect_templates_data_worker.get_page_text", lambda t, **kwargs: "some wikitext"
    )
    monkeypatch.setattr("src.main_app.jobs_workers.collect_templates_data_worker.find_main_title", lambda x: "file.svg")
    monkeypatch.setattr(
...
def test_fix_nested_main_files_worker_cancellation(mock_common_services, monkeypatch: pytest.MonkeyPatch):
    ...
    fix_nested_main_files_worker.fix_nested_main_files_for_templates(job_id=1, user=None, cancel_event=cancel_event)
