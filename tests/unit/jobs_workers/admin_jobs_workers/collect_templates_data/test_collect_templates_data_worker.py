...
def test_worker_class_get_job_type():
    ...
    _worker = worker.CollectMainFilesWorker(JobsRunner(job_id=1, user={}, cancel_event=threading.Event()))
...
def test_worker_init_update_all_can_be_set_true():
    ...
    _worker = worker.CollectMainFilesWorker(
        JobsRunner(job_id=1, user={}, cancel_event=threading.Event(), args={"update_all": "true"})
    )
