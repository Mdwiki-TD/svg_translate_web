...
Classes to test: UseRecord, AdminUserRecord, UserTokenRecord
"""
...
def test_users_record(mock_app: Flask) -> None:
    with mock_app.app_context():
        user = UseRecord(user_id=42, username="model_test_user")
        db.session.add(user)
        db.session.commit()
...
def test_admin_user_record(mock_app: Flask) -> None:
    with mock_app.app_context():
        user = UseRecord(user_id=1, username="model_admin_user")
        db.session.add(user)
        db.session.commit()
...
def test_user_token_record(mock_app: Flask) -> None:
    with mock_app.app_context():
        user = UseRecord(user_id=123, username="model_token_user")
        db.session.add(user)
        db.session.commit()
...
def test_user_token_record_validation(mock_app: Flask) -> None:
    with mock_app.app_context():
        user = UseRecord(user_id=456, username="model_validation_user")
        db.session.add(user)
        db.session.commit()
