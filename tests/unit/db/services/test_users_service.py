...
from src.main_app.db.models import UseRecord
...
@pytest.fixture
def user_record() -> UseRecord:
    """Insert and return a fresh UseRecord in the real test DB."""
    record = UseRecord(username="test_user")
...
class TestListUsers:
    ...
    def test_returns_all_users(self, user_record: UseRecord) -> None:
...
class TestGetUser:
    ...
    def test_returns_user_by_valid_id(self, user_record: UseRecord) -> None:
...
class TestGetUserByUsername:
    ...
    def test_returns_user_by_existing_username(self, user_record: UseRecord) -> None:
...
class TestCreateUser:
    ...
    def test_creates_new_user(self) -> None:
        ...
        persisted = db.session.query(UseRecord).filter_by(username="new_user").first()
        assert persisted is not None
        assert persisted.user_id == result.user_id

    def test_returns_existing_user(self, user_record: UseRecord) -> None:
    ...
    def test_race_condition_returns_existing(self) -> None:
        ...
        """
        existing_user = MagicMock(spec=UseRecord)
...
class TestToggleCanRunJobs:
    ...
    def test_toggles_to_true(self, user_record: UseRecord) -> None:
        ...
        refreshed = db.session.get(UseRecord, user_record.user_id)
        assert refreshed is not None
        assert bool(refreshed.can_run_jobs) is True

    def test_toggles_to_false(self, user_record: UseRecord) -> None:
