...
class TestExtractGet:
    def test_returns_form(self, extract, monkeypatch, patch_render):
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.session", {"pop": lambda k, d="": d})
        result = extract.extract_translations()
        assert "form" in result

    def test_restores_filename_from_session(self, extract, monkeypatch, patch_render):
        session = {}
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.session", session)
        session[EXTRACT_FILENAME_KEY] = "test.svg"
        result = extract.extract_translations()
        assert "form" in result
...
class TestExtractPost:
    def test_empty_filename(self, extract, monkeypatch, patch_render):
        flashed = []
    ...
    def test_strips_file_prefix(self, extract, monkeypatch, tmp_path):
    ...
    def test_download_failure(self, extract, monkeypatch, patch_render, tmp_path):
        flashed = []
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.flash", lambda m, c: flashed.append((m, c)))
        mock_download = MagicMock()
    ...
    def test_extraction_error(self, extract, monkeypatch, patch_render, tmp_path):
        flashed = []
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.flash", lambda m, c: flashed.append((m, c)))
        mock_download = MagicMock()
        mock_download.return_value = {"result": "success", "path": str(tmp_path / "test.svg")}
    ...
    def test_invalid_translations_type(self, extract, monkeypatch, patch_render, tmp_path):
        flashed = []
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.flash", lambda m, c: flashed.append((m, c)))
        mock_download = MagicMock()
        mock_download.return_value = {"result": "success", "path": str(tmp_path / "test.svg")}
    ...
    def test_successful_extraction(self, extract, monkeypatch, patch_render, tmp_path):
        flashed = []
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.flash", lambda m, c: flashed.append((m, c)))
        mock_download = MagicMock()
        mock_download.return_value = {"result": "success", "path": str(tmp_path / "test.svg")}
    ...
    def test_languages_extracted(self, extract, monkeypatch, patch_render, tmp_path):
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.flash", MagicMock())
        mock_download = MagicMock()
        mock_download.return_value = {"result": "success", "path": str(tmp_path / "test.svg")}
        monkeypatch.setattr("src.main_app.public.main_routes.extract_routes.download_one_file", mock_download)
