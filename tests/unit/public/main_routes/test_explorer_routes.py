...
from src.main_app.public.main_routes import explorer_routes
...
class TestByTitleDownloaded:
    def test_renders_list(self, monkeypatch, patch_templates):
        monkeypatch.setattr(
...
class TestByTitleTranslated:
    def test_sets_compare_link(self, monkeypatch, patch_templates):
        monkeypatch.setattr(
            "src.main_app.public.main_routes.explorer_routes.get_files",
...
class TestByTitleNotTranslated:
    def test_filters_translated(self, monkeypatch, patch_templates):
...
class TestByTitle:
    def test_renders_information(self, monkeypatch, patch_templates):
        monkeypatch.setattr(
...
class TestMain:
    def test_lists_titles(self, tmp_path, monkeypatch, patch_templates):
        root = tmp_path / "data"
...
class TestServeMedia:
    def test_returns_directory(self, monkeypatch):
        ...
        explorer_routes.serve_media("title", "files", "file.svg")
        assert called[0][0].replace("\\", "/").endswith("base/title/files")
        assert called[0][1] == "file.svg"


class TestServeThumb:
    def test_prefers_cached_file(self, tmp_path, monkeypatch):
        base = tmp_path / "data"
    ...
    def test_falls_back_to_original(self, tmp_path, monkeypatch):
        base = tmp_path / "data"
        thumbs = tmp_path / "thumbs"
        (base / "topic" / "files").mkdir(parents=True)
        (thumbs / "topic" / "files").mkdir(parents=True)
        ...
        explorer_routes.serve_thumb("topic", "files", "missing.svg")
        path = responses[0][0].replace("\\", "/")
        assert "thumbs" not in path


class TestCompare:
    def test_renders_template(self, monkeypatch, patch_templates):
