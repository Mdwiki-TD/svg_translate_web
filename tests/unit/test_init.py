...
def test_create_app_none_config_raises(mock_environ):
    with pytest.raises(ValueError, match="config_class must be provided"):
        create_app(None)


def test_error_handlers_400_returns_html(app):
    with app.test_client() as client:
        resp = client.get("/api/test?malformed")
        assert resp.status_code in (400, 404, 405)


def test_error_handlers_404_returns_not_found(app):
    with app.test_client() as client:
        resp = client.get("/nonexistent-route-test-404")
        assert resp.status_code == 404


def test_error_handlers_405_returns_not_allowed(app):
    with app.test_client() as client:
        resp = client.post("/")
        assert resp.status_code in (405, 200)


def test_500_error_handler_exists(app):
    spec = app.error_handler_spec.get(None, {})
...
def test_csrf_protection_enabled(app):
    assert app.config.get("WTF_CSRF_ENABLED", True) is True or app.config.get("WTF_CSRF_CHECK_DEFAULT", True) is True


@patch("src.main_app.init_db")
def test_init_app_and_db_success(mock_init_db, mock_environ):
    mock_db = MagicMock()
...
def test_init_app_and_db_exception(mock_init_db, mock_environ):
    mock_db = MagicMock()
    mock_app = create_app(TestingConfig)
    mock_app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///:memory:"
    result = init_app_and_db(mock_app, mock_db)
