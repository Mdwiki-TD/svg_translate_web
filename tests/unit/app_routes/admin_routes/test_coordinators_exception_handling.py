...
def test_add_coordinator_catches_both_lookup_and_value_errors(monkeypatch, caplog):
    ...
    mock_service = Mock()
    monkeypatch.setattr("src.main_app.app_routes.admin_routes.coordinators.admin_service", mock_service)

    # Mock Flask globals to avoid "Working outside of request context" errors
    mock_request = Mock()
    mock_request.form.get.return_value = "test_user"
    monkeypatch.setattr("src.main_app.app_routes.admin_routes.coordinators.request", mock_request)

    mock_flash = Mock()
    monkeypatch.setattr("src.main_app.app_routes.admin_routes.coordinators.flash", mock_flash)

    mock_redirect = Mock()
    monkeypatch.setattr("src.main_app.app_routes.admin_routes.coordinators.redirect", mock_redirect)

    monkeypatch.setattr("src.main_app.app_routes.admin_routes.coordinators.url_for", lambda x: f"/{x}")
