"""Unit tests for jobs_routes_utils.py shared job route handlers."""
...
from src.main_app.app_routes.public_jobs import JobsPublicRoutes
from src.main_app.db.exceptions import DuplicateJobError

BP_NAME = "public_jobs"
MODULE_PATH = "src.main_app.app_routes.jobs_routes_utils"
...
class TestCancelJob:
    def _setup_mocks(self, monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.flash", flash)
        monkeypatch.setattr(f"{MODULE_PATH}.redirect", redirect)
        monkeypatch.setattr(f"{MODULE_PATH}.url_for", url_for)
    ...
    def test_not_logged_in(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.load_user", lambda: None)

        result = cancel_job_handler(1, "test_job", BP_NAME)
    ...
    def test_job_not_found(self, monkeypatch: pytest.MonkeyPatch, mock_user: MagicMock) -> None:
        mocks = self._setup_mocks(monkeypatch)
        monkeypatch.setattr(f"{MODULE_PATH}.load_user", lambda: mock_user)
        monkeypatch.setattr(f"{MODULE_PATH}.get_job", MagicMock(side_effect=LookupError("not found")))
    ...
    def test_no_permission(self, monkeypatch: pytest.MonkeyPatch, mock_user: MagicMock, mock_job: MagicMock) -> None:
        mocks = self._setup_mocks(monkeypatch)
        monkeypatch.setattr(f"{MODULE_PATH}.load_user", lambda: mock_user)
        monkeypatch.setattr(f"{MODULE_PATH}.get_job", lambda jid, jt: mock_job)
        monkeypatch.setattr(f"{MODULE_PATH}.can_manage_job", lambda j, u: False)
    ...
    def test_cancel_successful(
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.load_user", lambda: mock_user)
        monkeypatch.setattr(f"{MODULE_PATH}.get_job", lambda jid, jt: mock_job)
        monkeypatch.setattr(f"{MODULE_PATH}.can_manage_job", lambda j, u: True)
...
class TestDeleteJob:
    ...
    def test_delete_successful(
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.delete_job", lambda jid, jt: True)

        result = delete_job_handler(1, "test_job", BP_NAME)
    ...
    def test_delete_failure(self, monkeypatch: pytest.MonkeyPatch, mock_user: MagicMock, mock_job: MagicMock) -> None:
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.can_manage_job", lambda j, u: True)
        monkeypatch.setattr("src.main_app.jobs_workers.jobs_worker.cancel_job_worker", lambda jid, jt: False)
        monkeypatch.setattr(f"{MODULE_PATH}.delete_job", lambda jid, jt: False)
    ...
    def test_exception_during_delete(
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.can_manage_job", lambda j, u: True)
        monkeypatch.setattr("src.main_app.jobs_workers.jobs_worker.cancel_job_worker", lambda jid, jt: False)
        monkeypatch.setattr(f"{MODULE_PATH}.delete_job", MagicMock(side_effect=RuntimeError("DB error")))
...
class TestStartJob:
    ...
    def test_not_logged_in(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = start_job_handler("test_job", {}, BP_NAME)
    ...
    def test_auth_payload_failure(self, monkeypatch: pytest.MonkeyPatch, mock_user: MagicMock) -> None:
        mocks = self._setup_mocks(monkeypatch)
        monkeypatch.setattr(f"{MODULE_PATH}.load_user", lambda: mock_user)
        monkeypatch.setattr(f"{MODULE_PATH}.load_auth_payload", MagicMock(side_effect=RuntimeError("OAuth error")))
    ...
    def test_duplicate_job_error(self, monkeypatch: pytest.MonkeyPatch, mock_user: MagicMock) -> None:
        mocks = self._setup_mocks(monkeypatch)
        monkeypatch.setattr(f"{MODULE_PATH}.load_user", lambda: mock_user)
        monkeypatch.setattr(f"{MODULE_PATH}.load_auth_payload", lambda u: {"token": "abc"})
        monkeypatch.setattr(
...
class TestJobsList:
    def _setup_mocks(self, monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.flash", flash)
        monkeypatch.setattr(f"{MODULE_PATH}.render_template", render_template)
    ...
    def test_normal_listing(self, monkeypatch: pytest.MonkeyPatch, mock_template_data: MagicMock) -> None:
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.list_jobs", lambda limit, job_type: mock_jobs)

        result = jobs_list_handler("test_job", mock_template_data, BP_NAME)
    ...
    def test_listing_with_0_jobs(self, monkeypatch: pytest.MonkeyPatch, mock_template_data: MagicMock) -> None:
        mocks = self._setup_mocks(monkeypatch)
        monkeypatch.setattr(f"{MODULE_PATH}.list_jobs", lambda limit, job_type: [])
    ...
    def test_exception_falls_back_to_empty(
        ...
        mocks = self._setup_mocks(monkeypatch)
        monkeypatch.setattr(f"{MODULE_PATH}.list_jobs", MagicMock(side_effect=RuntimeError("DB error")))
...
class TestJobDetail:
    def _setup_mocks(self, monkeypatch: pytest.MonkeyPatch) -> dict[str, MagicMock]:
        ...
        monkeypatch.setattr(f"{MODULE_PATH}.url_for", url_for)
        monkeypatch.setattr(f"{MODULE_PATH}.render_template", render_template)
        return {"flash": flash, "redirect": redirect, "url_for": url_for, "render_template": render_template}
    ...
    def test_job_found_without_result(
        ...
        mocks = self._setup_mocks(monkeypatch)
        monkeypatch.setattr(f"{MODULE_PATH}.get_job", lambda jid, jt: mock_job)
        monkeypatch.setattr(f"{MODULE_PATH}.load_job_result", lambda rf: None)

        result = job_detail_handler(1, "test_job", mock_template_data, BP_NAME)
    ...
    def test_job_found_with_expand_all(
        ...
        result = job_detail_handler(1, "test_job", mock_template_data, BP_NAME, expand_all=True)
...
class TestJobsPublicRoutesRoutes:
    ...
    def _common_mocks(self, monkeypatch: pytest.MonkeyPatch, mock_user: MagicMock, mock_job: MagicMock) -> None:
        monkeypatch.setattr(f"{MODULE_PATH}.load_user", lambda: mock_user)
        monkeypatch.setattr(f"{MODULE_PATH}.get_job", lambda jid, jt: mock_job)
        monkeypatch.setattr(f"{MODULE_PATH}.list_jobs", lambda limit, job_type: [mock_job])
        monkeypatch.setattr(f"{MODULE_PATH}.can_manage_job", lambda job, user: True)
        monkeypatch.setattr(f"{MODULE_PATH}.load_auth_payload", lambda u: {"token": "abc"})
        monkeypatch.setattr("src.main_app.jobs_workers.jobs_worker.cancel_job_worker", lambda *a: True)
        monkeypatch.setattr("src.main_app.jobs_workers.jobs_worker.start_job", lambda au, jt, args: 42)
        monkeypatch.setattr(f"{MODULE_PATH}.delete_job", lambda jid, jt: True)
        monkeypatch.setattr(f"{MODULE_PATH}.load_job_result", lambda rf: {"result": "ok"})
