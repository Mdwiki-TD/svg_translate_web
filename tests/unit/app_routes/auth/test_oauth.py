...
from src.main_app.su_services import mwoauth_handshake as oauth

MODULE = "src.main_app.su_services.mwoauth_handshake"
...
def test_get_handshaker_without_config(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(f"{MODULE}.settings", types.SimpleNamespace(oauth=None))
...
def test_start_login(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(f"{MODULE}.url_for", fake_url_for)
    monkeypatch.setattr(f"{MODULE}.get_handshaker", lambda: DummyHandshaker())
