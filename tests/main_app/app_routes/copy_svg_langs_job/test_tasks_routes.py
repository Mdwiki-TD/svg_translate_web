...
from src.main_app.public_jobs_workers.copy_svg_langs import routes
...
def app_client(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes._task_store", store_factory)
...
def test_task_renders_context_with_missing_task(
    ...
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes.current_user", lambda: None)
    ...
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes.render_template", fake_render)
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes.flash", fake_flash)
...
def test_task2_includes_ordered_stages(
    ...
    monkeypatch.setattr(
        "src.main_app.public_jobs_workers.copy_svg_langs.routes.current_user",
...
def test_start_creates_task_and_launches_thread(
    ...
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes.current_user", lambda: user)
...
def test_start_redirects_to_existing_task_when_duplicate(
    ...
    launch_calls: list[tuple[Any, ...]] = []
    monkeypatch.setattr(
        "src.main_app.public_jobs_workers.copy_svg_langs.routes.start_copy_svg_langs_job",
...
def test_tasks_renders_formatted_tasks(
    ...
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes.format_task", fake_format_task)
    monkeypatch.setattr(
        "src.main_app.public_jobs_workers.copy_svg_langs.routes.format_task_message", fake_format_message
    )
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes.render_template", fake_render)
