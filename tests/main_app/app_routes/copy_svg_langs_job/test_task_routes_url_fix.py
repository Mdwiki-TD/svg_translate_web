...
def app_client(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.routes._task_store", lambda: store)
...
def test_start_redirects_to_correct_task_endpoint(
    ...
    assert response.headers["Location"].endswith(f"/tasks/{existing_id}/info?title=Test+Title")
