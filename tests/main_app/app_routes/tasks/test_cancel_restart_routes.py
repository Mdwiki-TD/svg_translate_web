...
def app(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.settings", settings)

    monkeypatch.setattr("src.main_app.app_routes.steps.routes.flash", lambda *args, **kwargs: None)
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.jsonify", lambda payload: payload)
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.redirect", lambda url: {"redirect_to": url})
    monkeypatch.setattr(
        "src.main_app.app_routes.steps.routes.url_for",
...
def test_cancel_happy_path(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.main_app.app_routes.steps.routes._task_store", lambda: DummyStore())
    monkeypatch.setattr(
        "src.main_app.app_routes.steps.routes.current_user", lambda: types.SimpleNamespace(username="user")
    )
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.active_coordinators", lambda: ["user"])
    monkeypatch.setattr(
        "src.main_app.app_routes.steps.routes.get_cancel_event", lambda task_id, store=None: DummyEvent()
    )
...
def test_restart_creates_new_task(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.get_store_task", fake_get_store_task)
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.create_new_task", fake_create_new_task)
    monkeypatch.setattr(
        routes,
        "current_user",
        lambda: types.SimpleNamespace(user_id=1, username="user", access_token="tok", access_secret="sec"),
    )
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.parse_args", fake_parse_args)
    monkeypatch.setattr(
        "src.main_app.app_routes.steps.routes.uuid",
        types.SimpleNamespace(uuid4=lambda: types.SimpleNamespace(hex="newtask")),
    )
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.launch_task_thread", fake_launch)
...
def test_cancel_task_wrong_owner(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.active_coordinators", list)
...
def test_restart_task_collision(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.main_app.app_routes.steps.routes.parse_args", lambda f, d: types.SimpleNamespace())
