...
def test_translations_task_stops_on_failure(monkeypatch, tmp_path, extract_return, expected_message):
    ...
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.steps.extract.extract_task.download_one_file", fake_download_one_file)
    monkeypatch.setattr("src.main_app.public_jobs_workers.copy_svg_langs.steps.extract.extract_task.extract", fake_extract)
