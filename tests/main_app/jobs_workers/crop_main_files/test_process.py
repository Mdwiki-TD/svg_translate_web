...
def mock_services(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "src.main_app.jobs_workers.crop_main_files.process.settings",
        mock_settings
    )
...
def test_upload_one_success(mock_services):
    ...
    file_info = {
        ...
        "output_path": Path("/tmp/test_cropped.svg"),
        "status": "pending",
    }
...
def test_upload_one_skips_if_no_path():
    """Test upload is not attempted if output_path is None."""
    job_id = 1
    file_info = {
        "cropped_filename": "File:test (cropped).svg",
        "output_path": None,
