...
class TestLoadPage:
    def test_returns_page_on_success(self, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
        result = mw.load_page()
        assert result == mock_page

    def test_caches_page_on_second_call(self, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
        mw.load_page()
        mw.load_page()
    ...
    def test_invalid_page_title(self, mw, mock_site):
        mock_site.pages.__getitem__.side_effect = mwclient.errors.InvalidPageTitle("bad title")
        result = mw.load_page()
        assert result is False
        assert mw.load_page_error == "invalidpagetitle"

    def test_generic_exception(self, mw, mock_site):
        mock_site.pages.__getitem__.side_effect = Exception("connection error")
        result = mw.load_page()
        assert result is False
        assert mw.load_page_error == "connection error"


# ── check_exists ───────────────────────────────────────────────────────────────

class TestCheckExists:
    def test_page_exists(self, mw, mock_site, mock_page):
        mock_page.exists = True
        mock_site.pages.__getitem__.return_value = mock_page
        assert mw.check_exists() is True

    def test_page_does_not_exist(self, mw, mock_site, mock_page):
        mock_page.exists = False
        mock_site.pages.__getitem__.return_value = mock_page
        assert mw.check_exists() is False

    def test_load_page_fails(self, mw, mock_site):
...
class TestEditPageInternal:
    def test_success(self, mw, mock_page):
        mock_page.edit.return_value = None
        result = mw._edit_page(mock_page, "text", "summary")
        assert result == {"success": True}

    def test_edit_error(self, mw, mock_page):
    ...
    def test_protected_page_error(self, mw, mock_page):
    ...
    def test_unexpected_exception(self, mw, mock_page):
...
class TestEditPage:
    def test_success(self, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
        mock_page.edit.return_value = None
        result = mw.edit_page("text", "summary")
        assert result == {"success": True}

    def test_load_page_fails_invalid_title(self, mw, mock_site):
        mock_site.pages.__getitem__.side_effect = mwclient.errors.InvalidPageTitle("x")
        result = mw.edit_page("text", "summary")
        assert result["success"] is False
        assert result["error"] == "invalidpagetitle"

    def test_load_page_fails_generic(self, mw, mock_site):
        mock_site.pages.__getitem__.side_effect = Exception("network error")
        result = mw.edit_page("text", "summary")
        assert result["success"] is False
        assert result["error"] == "network error"

    def test_edit_error_no_retry(self, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
        mock_page.edit.side_effect = mwclient.errors.EditError(mock_page, {})
        result = mw.edit_page("text", "summary")
        assert result["success"] is False
    ...
    def test_protected_page_no_retry(self, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
    ...
    def test_ratelimited_then_success(self, mock_sleep, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
    ...
    @patch("src.main_app.api_services.mwclient_page.time.sleep")
    def test_ratelimited_exhausts_all_retries(self, mock_sleep, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
        rate_exc = mwclient.errors.APIError("ratelimited", "Rate limited", {})
    ...
    @patch("src.main_app.api_services.mwclient_page.time.sleep")
    def test_ratelimited_then_other_api_error(self, mock_sleep, mw, mock_site, mock_page):
        mock_site.pages.__getitem__.return_value = mock_page
        rate_exc = mwclient.errors.APIError("ratelimited", "Rate limited", {})
    ...
    @patch("src.main_app.api_services.mwclient_page.time.sleep")
    def test_retry_sleep_delays_are_correct(self, mock_sleep, mw, mock_site, mock_page):
        """Verify the exact delay sequence: 5s, 15s, 30s."""
        mock_site.pages.__getitem__.return_value = mock_page
        rate_exc = mwclient.errors.APIError("ratelimited", "Rate limited", {})
        mock_page.edit.side_effect = rate_exc
        mw.edit_page("text", "summary")
