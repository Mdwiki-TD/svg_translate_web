    ...
    try:
        ...
        for template in templates:
            ...
            try:
                ...
                logger.info(f"Job {job_id}: Processing {template.main_file} for template {template.title}")
                ...
                if fix_result["success"]:
                    ...
                    result["templates_failed"].append(template_info)
                    result["summary"]["failed"] += 1
                    logger.warning(f"Job {job_id}: Failed to process {template.main_file}: {fix_result.get('message')}")
