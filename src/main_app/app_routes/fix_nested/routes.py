...
def oauth_required_with_filename_preservation(func: F) -> F:  # noqa: UP047
    """Custom OAuth decorator that preserves filename in session before redirect."""

    @wraps(func)
    def wrapper(*args: Any, **kwargs: Any):
        if not current_user() and not settings.is_localhost(request.host):
...
@bp_fix_nested.route("/tasks/<task_id>")
...
@bp_fix_nested.route("/tasks/<task_id>/compare")
...
@bp_fix_nested.route("/tasks/<task_id>/undo", methods=["POST"])
@admin_required
