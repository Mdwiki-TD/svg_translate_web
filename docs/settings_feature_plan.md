...
- تنفيذ الـ SQL Script لإنشاء جدول الإعدادات الجديد الذي سيتضمن هيكلة المفاتيح (keys) وأنواع القيم (types).
- الاستعلام:
...
- **مسار الإضافة (Create Route)**: نقطة لاستقبال طلبات إضافة إعداد جديد وتمرير الـ (key, title, value_type, value) وحفظه في الجدول، مع ضمان عدم تكرار الـ `key`.
- **مسار التحديث (Update Route)**: مسار لاستقبال التحديثات من واجهة الإعدادات. يقوم بمعالجة الحقول بحسب نوعها وتحديث الـ `value` لكل `key` موجود في الجدول.
...
- **قائمة الإعدادات الحالية (List & Edit Settings)**:
  - سيتم جلب الإعدادات في الواجهة وعرض كل `title` بجواره حقل تعديل الـ `value` يتغير نوعه ديناميكياً بناءً على حقل `value_type`:
...
- تحديث الأجزاء المبرمجة لتعمـــل بالاعتماد على القيم المحمّلة في `config` والمستمدة من مفاتيح الإعدادات (`key`) الموجودة بالنظام.
- **التخزين المؤقت (Caching)**: تفعيل نظام التخزين المؤقت للإعدادات بحيث يتم قراءتها من הـ Cache أثناء تشغيل التطبيق بدلاً من الجدول باستمرار، ولا يُفرَّغ التخزين (Cache Invalidation) إلا بعد قيام الأدمن بالتعديل في صفحة الإعدادات وحفظها.
